...
export default function DayScreen() {
  ...
  const bloodMoonInGame = selectedRoles.some(entry => entry.roleId === 'blood_moon');
  ...
  const delayedDeathAnnouncementRequired =
    bloodMoonInGame && (bloodMoonActivated || resolvedNightDeathRounds.length === 0);
  ...
  // 血月延後技能觸發（step 4）
  const [resultDayDeathsAnnounced, setResultDayDeathsAnnounced] = useState(false);
  ...
  const delayedNightDeathRounds = delayedDeathAnnouncementRequired
    ? resolveAutomaticDeathRounds(
        directNightDeaths,
        'night',
        nightActions,
  ...
  const bmHunterDeathRounds = bmHunterResolved && resolvedBmHunterTarget !== undefined
    ? resolveAutomaticDeathRounds(
        [resolvedBmHunterTarget],
        'night',
        nightActions,
        roleMembersMap,
        playerCardMap,
  ...
  const bmWolfKingDeathRounds = bmWolfKingResolved && resolvedBmWolfKingTarget !== undefined
    ? resolveAutomaticDeathRounds(
        [resolvedBmWolfKingTarget],
        'night',
        nightActions,
        roleMembersMap,
        playerCardMap,
  ...
  const unannouncedBloodMoonSkillDeathRounds = bloodMoonSkillDeathRounds.slice(
    announcedBloodMoonSkillRoundCount,
  );
  const completeDelayedNightDeathRounds = [
  ...
  const bloodMoonDeathSkillTriggers = delayedDeathAnnouncementRequired
    ? getDeathSkillTriggers(
  ...
  // ── Step 2: 發言環節 ──────────────────────────────────────────────────
  const renderDayDeathSkill = (showVoteHeader = false) => {
    ...
    return (
      <ScrollView style={styles.scroll} contentContainerStyle={[styles.scrollPad, { flexGrow: 1 }]}>
        {showVoteHeader && (
          <View style={[styles.banner, { borderColor: Colors.warning, marginBottom: 8 }]}>
            <Text style={[styles.bannerText, { color: Colors.warning }]}>🗳️ 票逐資訊</Text>
            <Text style={styles.hint}>處理票逐死訊與死亡角色技能</Text>
          </View>
        )}
        <View style={styles.skillHeader}>
  ...
  const renderSpeech = () => {
    ...
    if (speechDeathNotice !== null) {
      ...
      return (
        <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollPad}>
          <View style={[styles.banner, styles.bannerDanger]}>
            <Text style={styles.bannerText}>白天死訊</Text>
          </View>
          ...
          <TouchableOpacity
            ...
            onPress={() => {
              ...
              if (speechDeathNotice === 'knight_wolf' && delayedDeathAnnouncementRequired) {
                setLocalSheriff(finalSheriff);
                setExiledPlayer(undefined);
                setSpeechDeathNotice(null);
                setStep(4);
  ...
  const renderResult = () => {
    ...
    const sheriffKilledByBloodMoon =
      delayedDeathAnnouncementRequired &&
      localSheriff !== null &&
      completeDelayedNightDeaths.includes(localSheriff) &&
      !sheriffExiled &&
    ...
    const targetNums   = aliveNums.filter(p =>
      !completeDelayedNightDeaths.includes(p) && (isIdiotFlip || p !== exiledPlayer)
    );
    ...
    const bloodMoonCandidates = playerNums.filter(p => {
      if (deadPlayers.includes(p)) return false;
      if (resolvedDayDeaths.includes(p)) return false;
      if (completeDelayedNightDeaths.includes(p)) return false;
    ...
    const renderVoteHeader = (detail: string) => (
      <View style={[styles.banner, { borderColor: Colors.warning, marginTop: 8 }]}>
        <Text style={[styles.bannerText, { color: Colors.warning }]}>🗳️ 票逐資訊</Text>
        {detail !== '' && <Text style={[styles.hint, { color: Colors.text, marginTop: 4 }]}>{detail}</Text>}
      </View>
    );
    const renderBloodMoonHeader = (detail: string) => (
      <View style={[styles.banner, { borderColor: Colors.wolf, marginTop: 8 }]}>
        <Text style={[styles.bannerText, { color: Colors.wolf }]}>🌑 血月延後資訊</Text>
        {detail !== '' && <Text style={[styles.hint, { color: Colors.text, marginTop: 4 }]}>{detail}</Text>}
      </View>
    );
    const renderBloodMoonInfo = () => (
      <>
        {renderBloodMoonHeader('')}
        {completeDelayedNightDeathRounds.length === 0 && (
          <Text style={[styles.hint, { textAlign: 'center' }]}>昨晚平安夜</Text>
        )}
        {bearTamerResult && (
          <Text style={[styles.hint, { color: Colors.text, textAlign: 'center' }]}>
    ...
    const renderBloodMoonHunterSkill = () => (
      <>
        <View style={styles.skillHeader}>
    ...
    const renderBloodMoonWolfKingSkill = () => (
      <>
        <View style={styles.skillHeader}>
    ...
    if (pendingDayDeathSkill) return renderDayDeathSkill(true);
    ...
    if (needsResultDayDeathAnnouncement && !resultDayDeathsAnnounced) {
      return (
        <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollPad}>
          ...
          <View style={[styles.banner, { borderColor: Colors.textMuted, marginTop: 8 }]}>
            <Text style={styles.hint}>票逐死訊已公布，確認後繼續處理相關操作</Text>
          </View>
        </ScrollView>
      );
    }

    const voteSheriffDeathPlayer = sheriffExiled
      ? exiledPlayer as number
      : sheriffKilledByDayChain
      ? localSheriff
      : null;
    const needsVoteBadge =
      voteSheriffDeathPlayer !== null &&
      !(sheriffExiled && isIdiotFlip) &&
      !exileBadgeResolved;
    ...
    if (needsVoteBadge) {
      return (
        <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollPad}>
          {renderVoteHeader('處理票逐死亡造成的警徽移交')}
          {renderBadgeUI(
            voteSheriffDeathPlayer,
    ...
    if ((bishopDiedExile || bishopDiedDayChain) && !bishopResolved) {
      return (
        <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollPad}>
          {renderVoteHeader('處理票逐死亡角色技能')}
          {renderBishopUI()}
        </ScrollView>
      );
    }
    ...
    if (delayedDeathAnnouncementRequired && !bloodMoonDeathsAnnounced) {
      return (
        <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollPad}>
          {renderBloodMoonInfo()}
          {bmHunterPlayer !== undefined && !bmHunterResolved
            ? renderBloodMoonHunterSkill()
            : bmWolfKingPlayer !== undefined && !bmWolfKingResolved
            ? renderBloodMoonWolfKingSkill()
            : null}
        </ScrollView>
      );
    }

    if (unannouncedBloodMoonSkillDeathRounds.length > 0) {
      return (
        <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollPad}>
          {renderBloodMoonInfo()}
          <View style={[styles.banner, { borderColor: Colors.textMuted }]}>
            <Text style={styles.hint}>新增死訊已公布，確認後繼續處理死亡角色技能</Text>
          </View>
        </ScrollView>
      );
    }

    if (delayedDeathAnnouncementRequired && bmHunterPlayer !== undefined && !bmHunterResolved) {
      return (
        <ScrollView style={styles.scroll} contentContainerStyle={[styles.scrollPad, { flexGrow: 1 }]}>
          {renderBloodMoonInfo()}
          {renderBloodMoonHunterSkill()}
    ...
    if (delayedDeathAnnouncementRequired && bmWolfKingPlayer !== undefined && !bmWolfKingResolved) {
      return (
        <ScrollView style={styles.scroll} contentContainerStyle={[styles.scrollPad, { flexGrow: 1 }]}>
          {renderBloodMoonInfo()}
          {renderBloodMoonWolfKingSkill()}
    ...
    if (delayedDeathAnnouncementRequired && bishopDiedBloodMoon) {
      return (
        <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollPad}>
          {renderBloodMoonInfo()}
          {renderBishopUI()}
  ...
  const handleNext = () => {
    ...
    if (pendingDayDeathSkill) {
      ...
      if (pendingDayDeathSkill.player === exiledPlayer) {
        ...
        setExileAbilityTarget(dayDeathSkillTarget);
        setExileAbilityResolved(true);
      }
    ...
    if ((bishopDiedExile || bishopDiedDayChain) && !bishopResolved && bishopRevealTarget !== null) {
      setBishopHolder(bishopRevealTarget);
    ...
    if (delayedDeathAnnouncementRequired && !bloodMoonDeathsAnnounced) {
      ...
      if (bmHunterPlayer !== undefined && !bmHunterResolved) {
        setBmHunterResolved(true);
      } else if (bmWolfKingPlayer !== undefined && !bmWolfKingResolved) {
        setBmWolfKingResolved(true);
      }
      return;
    }
    if (unannouncedBloodMoonSkillDeathRounds.length > 0) {
      setAnnouncedBloodMoonSkillRoundCount(bloodMoonSkillDeathRounds.length);
      return;
    }
    if (delayedDeathAnnouncementRequired && bmHunterPlayer !== undefined && !bmHunterResolved) {
      setBmHunterResolved(true);
    ...
    if (delayedDeathAnnouncementRequired && bishopDiedBloodMoon && bishopRevealTarget !== null) {
      setBishopHolder(bishopRevealTarget);
      setBishopResolved(true);
      return;
    }
  ...
  const canAdvance = (): boolean => {
    ...
    if (step === 4) {
      ...
      const voteSheriffDied = sheriffKilledInDayChain || (sheriffExiled && !isIdiotFlip);
      if (voteSheriffDied && !exileBadgeResolved) {
  ...
  const nextLabel = (() => {
    ...
    if (step === 4 && delayedDeathAnnouncementRequired && !bloodMoonDeathsAnnounced) {
      ...
      if (bmWolfKingPlayer !== undefined && !bmWolfKingResolved) {
    ...
    if (step === 4 && unannouncedBloodMoonSkillDeathRounds.length > 0) {
      return '確認新增死訊，繼續';
    }
    if (step === 4 && delayedDeathAnnouncementRequired && bmHunterPlayer !== undefined && !bmHunterResolved) {
      return bmHunterTarget === null
        ? '放棄開槍並繼續'
        : resolvedBmHunterTarget === undefined
        ? '確認目標受保護並繼續'
    ...
    if (step === 4 && delayedDeathAnnouncementRequired && bmWolfKingPlayer !== undefined && !bmWolfKingResolved) {
      return bmWolfKingTarget === null
        ? '放棄帶人並繼續'
        : resolvedBmWolfKingTarget === undefined
        ? '確認目標受保護並繼續'
